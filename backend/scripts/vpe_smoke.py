...
_UPSCALE_SRC = Fixture(
    ...
    ["-f", "lavfi", "-i", f"testsrc=size={_LANDSCAPE}:rate=24:duration=6",
     "-pix_fmt", "yuv420p", "-c:v", "libx264"],
)
_TRANSFORM_SRC = Fixture(
    "transform_src.mp4",
    ["-f", "lavfi", "-i", f"testsrc=size={_LANDSCAPE}:rate=24:duration=8",
     "-pix_fmt", "yuv420p", "-c:v", "libx264"],
)
_MASK_SRC = Fixture(
    "mask.mp4",
    # A grayscale mask matching the source's size and length.
    ["-f", "lavfi", "-i", f"color=c=gray:size={_LANDSCAPE}:rate=24:duration=8",
     "-pix_fmt", "yuv420p", "-c:v", "libx264"],
)
_PERF_SRC = Fixture(
    "perf_actor.mp4",
    ...
    ["-f", "lavfi", "-i", f"testsrc=size={_LANDSCAPE}:rate=24:duration=8",
     "-frames:v", "192", "-pix_fmt", "yuv420p", "-c:v", "libx264"],
)
_STILL = Fixture(
    "still.png",
...
_VOICE = Fixture(
    "voice.wav",
    ["-f", "lavfi", "-i", "sine=frequency=220:duration=8", "-ac", "2",
     "-ar", "48000"],
)
_PORTRAIT = Fixture(
    "portrait_src.mp4",
    # For the orientation probe: the one question the docs cannot answer.
    ["-f", "lavfi", "-i", "testsrc=size=720x1280:rate=24:duration=6",
     "-pix_fmt", "yuv420p", "-c:v", "libx264"],
)
...
def _cases() -> list[Case]:
    ...
    def omni_cine(uris: dict[str, str], out: str) -> VpeRequest:
        return VpeRequest(
            ...
            video=VpeMediaRef(gcs_uri=uris["transform_src.mp4"],
                              mime_type=MP4),
        )
    ...
    def omni_cine_portrait(uris: dict[str, str], out: str) -> VpeRequest:
        return VpeRequest(
            ...
            storage_uri=out,
            prompt="A calm wide shot. Keep everything else the same.",
            video=VpeMediaRef(gcs_uri=uris["portrait_src.mp4"],
                              mime_type=MP4),
        )
    ...
    def transform_keyframes(uris: dict[str, str], out: str) -> VpeRequest:
        return VpeRequest(
            ...
            conditioning_frames=(
                VpeConditioningFrame(
                    image=VpeMediaRef(gcs_uri=uris["still.png"],
                                      mime_type=PNG),
                    frame_num=24,
                ),
            ),
    ...
    def upscale_seamless(uris: dict[str, str], out: str) -> VpeRequest:
        return VpeRequest(
            ...
            storage_uri=out,
            video=VpeMediaRef(gcs_uri=uris.get("__texture__", ""),
                              mime_type=MP4),
            resolution="4k",
            aspect_ratio="16:9",
            seamless=VpeSeamlessFlags(
    ...
    return [
        Case("upscale", VpeCapabilityId.UPSCALE, (_UPSCALE_SRC,), upscale,
             "The Phase 1 capability. 6s landscape, 24fps."),
        Case("upscale_portrait", VpeCapabilityId.UPSCALE, (_PORTRAIT,),
             upscale_portrait,
             "Confirms the documented portrait support really works."),
        Case("omni_cine", VpeCapabilityId.OMNI_CINE, (_TRANSFORM_SRC,),
             omni_cine, "Landscape baseline."),
        Case("omni_cine_portrait", VpeCapabilityId.OMNI_CINE, (_PORTRAIT,),
             omni_cine_portrait,
        ...
        Case("video_transform", VpeCapabilityId.VIDEO_TRANSFORM,
             (_TRANSFORM_SRC,), transform, "Whole-frame restyle."),
        Case("video_transform_masked", VpeCapabilityId.VIDEO_TRANSFORM,
             (_TRANSFORM_SRC, _MASK_SRC), transform_masked,
             "Localised edit via a grayscale mask video."),
        Case("video_transform_keyframes", VpeCapabilityId.VIDEO_TRANSFORM,
             (_STILL,), transform_keyframes,
             "Multi-keyframe I2V; frameNum must be a multiple of 8."),
        Case("perf_estimation", VpeCapabilityId.PERF_ESTIMATION, (_PERF_SRC,),
             perf_estimation, "Produces the blue mesh.", feeds="__blue_mesh__"),
        Case("perf_generation", VpeCapabilityId.PERF_GENERATION, (_STILL,),
             perf_generation, "Consumes the blue mesh.",
             depends_on="perf_estimation"),
        Case("dialogue_driven", VpeCapabilityId.DIALOGUE_DRIVEN,
             (_STILL, _VOICE), dialogue,
             "Highest creative value: lip-sync to a supplied track."),
        Case("video_textures", VpeCapabilityId.VIDEO_TEXTURES, (), textures,
             "Seamless loop plus tessellation.", feeds="__texture__"),
        Case("upscale_seamless", VpeCapabilityId.UPSCALE_SEAMLESS, (),
             upscale_seamless,
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    code, out = _run(["gcloud", "projects", "describe", project,
    ...
    code, out = _run(["gcloud", "services", "list", "--enabled",
    ...
    code, out = _run(["gcloud", "storage", "buckets", "describe", bucket,
                      "--format=value(name)"])
    if code == 0:
    ...
    code, policy = _run(["gcloud", "storage", "buckets", "get-iam-policy",
                         bucket, "--format=json"])
    for agent in _SERVICE_AGENTS:
        ...
        if member in policy:
            ...
            ok = False
            print(f"  FAIL  {agent} has NO binding. If this principal does "
                  f"not exist yet, start and immediately cancel a batch "
                  f"prediction (bp) or tuning (tune) job first, then:")
            print(f"          gcloud storage buckets add-iam-policy-binding "
                  f"{bucket} \\\n            --member=serviceAccount:{member} "
                  f"\\\n            --role=roles/storage.admin")

    print("\nallowlist probe (a 400 means you ARE allowlisted; 403/404 means "
          "you are not):")
    client = VpeClient(project_id=project, location=args.location,
                       dry_run=False)
    ...
    if code == 0:
        ...
        _, status = _run(probe)
        verdict = {"400": "ALLOWLISTED (reached the model)",
                   "403": "NOT allowlisted",
                   "404": "NOT allowlisted (model not found)"}.get(
                       status.strip(), "unexpected")
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.print_curl:
        ...
        uris["__blue_mesh__"] = f"{bucket}/vpe-smoke/out/perf_estimation/" \
                                "sample_0.mp4"
        uris["__texture__"] = f"{bucket}/vpe-smoke/out/video_textures/" \
                              "sample_0.mp4"
    ...
    for case in selected:
        ...
        try:
            for fixture in case.fixtures:
                ...
                if fixture.name not in uris and not args.print_curl:
                    ...
                    uris.setdefault(
                        fixture.name,
                        f"{bucket}/vpe-smoke/in/{fixture.name}")
            ...
            rows.append({"case": case.key,
                         "model": case.capability_id.value,
                         "status": "BUILD_FAILED", "detail": str(exc),
                         "seconds": 0, "output": ""})
            _write_csv(csv_path, rows)
            continue
        ...
        try:
            ...
            ) if hasattr(client, "wait_for_completion_sync") else _poll(
                client, operation, args.timeout)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    common(run)
    run.add_argument("--only", nargs="*", metavar="CASE",
                     help=f"one or more of: "
                          f"{', '.join(c.key for c in _cases())}")
    run.add_argument("--all", action="store_true",
                     help="every case (the default when --only is omitted)")
    run.add_argument("--print-curl", action="store_true",
                     help="render requests without sending; needs no "
                          "allowlist")
